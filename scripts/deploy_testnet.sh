...
Usage: STELLAR_SOURCE=<account> ./scripts/deploy_testnet.sh [--real-zk]
  ...
  --real-zk   UltraHonk verifier with utxo_actions VK (requires bb + VK built)
...
if [[ "$REAL_ZK" == true ]]; then
  VK="$ROOT/artifacts/utxo_actions/vk"
  if [[ ! -f "$VK" ]]; then
    echo "VK missing — run ./scripts/build_vk_utxo_actions.sh first" >&2
    exit 1
  fi
...
Relayer (optional):
  ...
  RELAYER_SECRET=<S> VAULT_ID=$VAULT_ID npm run server
