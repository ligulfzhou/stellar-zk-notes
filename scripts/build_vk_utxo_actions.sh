#!/usr/bin/env bash
# Compile utxo_actions circuit and write UltraHonk VK (keccak oracle).
set -euo pipefail
...
CIRCUIT="$ROOT/circuits/utxo_actions"
ARTIFACTS="$ROOT/artifacts/utxo_actions"
...
echo "==> nargo compile utxo_actions (nargo $ZK_NOIR_VERSION)"
nargo compile

JSON="./target/utxo_actions.json"
if [[ ! -f "$JSON" ]]; then
