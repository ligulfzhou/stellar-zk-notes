...
/** Walk a Merkle path and check it reaches `root` (matches utxo_actions verify_merkle_path). */
...
export async function merkleWitnessFromTreeState(params: {
  ...
  const ok = await verifyMerklePath(targetLeaf, path, indices, root);
  if (!ok) {
    throw new Error(
      `Merkle path does not verify for leaf ${targetIndex} (leafCount ${leafCount})`
    );
  }
